...
class TagDeleteCommand extends Command {
	...
	exec(message, { tag }) {
		...
		if (tag.user !== message.author.id && !staffRole) return message.channel.reply('you can only delete your own tags.');
		tag.destroy();

		return message.channel.reply(`successfully deleted **${tag.name}**.`);
	}
}
