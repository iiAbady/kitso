...
class TicketCommand extends Command {
	...
	async exec(message, args) {
		...
		if (args.first === 'help' || !args.first) {
			...
			try {
				var response = await message.channel.awaitMessages(msg2 => msg2.content === (`${this.handler.prefix(message)}t close` || msg2.content === 'cancel') && msg2.author.id === message.author.id, { maxMatches: 1, time: 20000, errors: ['time'] }); // eslint-disable-line no-var
			} catch (error) {
