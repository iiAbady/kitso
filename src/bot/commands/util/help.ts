...
export default class HelpCommand extends Command {
			...
			args: [
				{
					...
					type: (_, cmd): Command | string => {
						if (!cmd) return '';
						const commandAlias = this.handler.modules
							.filter((modulex): boolean => !modulex.ownerOnly)
							.find((commando): boolean => commando.aliases.includes(cmd));
	...
	public async exec(message: Message, { command }: { command: Command }): Promise<Message | Message[]> {
		...
		const embed = new MessageEmbed()
			.setColor(3447003)
			.setAuthor(
				`${command.aliases[0].replace(/(\b\w)/gi, (lc): string => lc.toUpperCase())} Help`,
				'https://images-ext-2.discordapp.net/external/Na1A42IsnllvKah5w2E8qEoTX5VMgkiFd6Y18oy7-Ws/%3Fwidth%3D473%26height%3D473/https/images-ext-2.discordapp.net/external/ixx9VwaXIvBi71wGahYe_NzG51gFQonnXVBl2eEbQmk/https/cdn.pixabay.com/photo/2012/04/14/16/26/question-34499_960_720.png',
			)
		...
		if (command.description.usage)
			embed.addField('❯ Usage', `\`${prefix}${command.aliases[0]} ${command.description.usage}\``, true);
