...
export default class ANISYNCCOMMAND extends Command {
	...
	public async exec(message: Message, { query }: { query: string }): Promise<Message | Message[]> {
		...
		}: { data: { Page: { media: [Anime] } } } = await (await fetch('https://graphql.anilist.co', {
			method: 'POST',
			headers: {
		...
		const { year: startYear, month: startMonth, day: startDay } = anime.startDate;

		const { year: endYear, month: endMonth, day: endDay } = anime.endDate;
		...
		const embed = new MessageEmbed()
			...
			.setFooter(
				`${this.safe(startYear)}/${this.safe(startMonth)}/${this.safe(startDay)} to ${this.safe(endYear)}/${this.safe(
					endMonth,
				)}/${this.safe(endDay)}`,
				'https://i.imgur.com/b7xjPhF.png',
			)
			.setColor('ORANGE');
