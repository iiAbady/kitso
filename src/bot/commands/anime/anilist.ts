...
export default class ANISYNCCOMMAND extends Command {
			...
			args: [
				{
					...
					type: Argument.compose(
						(_, str): string => str.replace(/\s/g, ''),
						Argument.range('number', 1, Infinity),
					),
					default: 1,
					unordered: true,
				},
	...
	public async exec(message: Message, { type, page, member }: { type: string; page: number; member: GuildMember }) {
		...
		const {
		...
		}: { data: { MediaListCollection: { lists: Array<List> } } } = await (await fetch('https://graphql.anilist.co', {
			method: 'POST',
			headers: {
			...
			body: JSON.stringify({
				query: `{
					MediaListCollection(userId: ${
						user.ani
					}, type: ${type.toUpperCase()}, status: COMPLETED, sort: SCORE_DESC, forceSingleCompletedList: true) {
						lists {
						  entries {
							score
							media {
							...
							**${type.replace(/(\b\w)/gi, (lc): string => lc.toUpperCase())} List:**
							...
							${
								list
									? list
											.map(
												item => `**${++index}.** [${item.media.title.romaji}](${item.media.siteUrl}) ― ${item.score}`,
											)
											.join('\n')
									: '*Nothing here*'
							}
