...
const AUTH_URL = 'https://anilist.co/api/v2/oauth/authorize?client_id=2090&redirect_uri=https://anilist.co/api/v2/oauth/pin&response_type=code';
...
export default class ANISYNCCOMMAND extends Command {
	...
	public async exec(message: Message): Promise<Message | Message[]> {
		...
		const { data: { Viewer: { id, name } } }: { data: { Viewer: { id: string; name: string } } } = await (await fetch('https://graphql.anilist.co', {
			method: 'POST',
			headers: {
